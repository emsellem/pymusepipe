...
from pymusepipe.init_musepipe import init_muse_parameters
from pymusepipe.graphcheck_pipe import check_plot
...
def feed_sof(sof_filename, folderin="", list_files=[], type_files="BIAS", verbose=True) :
    ...
    for filename in list_files :
        text_to_write = "{0} {1}\n".format(folderin + filename, type_files)
...
class muse_pipe(object):
   ...
   def create_calibrations(self):
    ...
    def check_for_calibrations(self, verbose=None):
        ...
        for mastertype in listMaster_dic.keys() :
            ...
            if mastertype == "TWILIGHT" :
                if not os.path.isfile("{outcal}{fout}/{suffix}.fits".format(outcal=outcal, fout=fout, suffix=suffix)):
                    self.Master[mastertype] = False
            ...
            # Others = 24 IFU exposures
                ...
                for ifu in range(1,25):
                    if not os.path.isfile("{0}{1}/{2}-{3:02d}.fits".format(outcal, fout, suffix, ifu)):
                        self.Master[mastertype] = False
                        break
                if verbose :
                    if self.Master[mastertype]:
    ...
    def run_bias(self, expoBIAS=None):
        ...
        safely_create_folder(self.my_params.mastercalib_folder + 'BIAS')
        muse_bias.output_dir = self.my_params.mastercalib_folder + 'BIAS'
        muse_bias.calib.BADPIX_TABLE = self.my_params.musecalib_folder + self.badpix_table
        muse_bias(self.BIAS)
